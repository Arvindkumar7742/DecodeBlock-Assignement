...
export const Main = () => {
    ...
    function handleOnDrop(e) {
        ...
        if (sideFlag) {
            ...
            x = x - offset_x;
            y = y - offset_y;
            setData({ text: "", fontSize: "16", fontWeight: "", x, y, type ,options:[]});
            setOpenModal(true);
            return;
        }
    ...
    return (
        ...
        >
            ...
            {draggedElements && draggedElements.map((item, index) => (
                ...
                >
                    {item.type === "Label" && <Label text={item.text}/>}
                    {item.type === "Input" && (<Input item={item} handleInputChange={handleInputChange}/>)}
                    {item.type === 'Button' && (<Button text={item.text}/>)}
                    {item.type === "Select" && (<Select item={item} handleSelectChange={handleSelectChange}/>)}
                </div>
            ))}
